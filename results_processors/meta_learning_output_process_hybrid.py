...
def main():
    ...
    results_map = pd.DataFrame({
        "leaf": ["union_3", "union_4", "union_6", "union_7"],
        #"encode": np.zeros(4),
        "features": np.zeros(4),
        #"impute": np.zeros(4),
    ...
    results_map.to_csv(os.path.join(result_path, "meta_learning_output_process_hybrid.csv"), index=False)

    fig, axes = plt.subplots(nrows=1, ncols=1)
    results_map.plot(ax=axes, kind='bar', x='leaf')
    fig.savefig(os.path.join(result_path, "meta_learning_output_process_hybrid.pdf"), bbox_inches='tight')
