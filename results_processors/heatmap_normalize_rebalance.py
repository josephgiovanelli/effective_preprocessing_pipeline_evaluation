...
def main():
    ...
    for algorithm in algorithms:
        results_map[algorithm] = pd.DataFrame({
                        "RN": ["NoneType", "NearMiss", "SMOTE"], 
                        "NoneType": [0, 0, 0],
                        "StandardScaler": [0, 0, 0],
                        "PowerTransformer": [0, 0, 0],
                        "MinMaxScaler": [0, 0, 0],
                        "RobustScaler": [0, 0, 0]
                        })
        results_map[algorithm] = results_map[algorithm].set_index('RN')
            ...
            if acronym in results:
                with open(os.path.join(input_auto, acronym + '.json')) as json_file:
                    ...
                    try:
                        ...
                        results_map[algorithm].loc[rebalance_flag, normalize_flag] += 1
                    except:
                        print()
    ...
    for algorithm in algorithms:
        results_map[algorithm].to_csv(os.path.join(meta_learning_input_path, 'heatmap_rebalance_normalize_' + algorithm + '.csv'), index=True)
